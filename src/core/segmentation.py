...
class SegmentationEngine:
    ...
    def run_vitmatte(self, img_np, mask_np, w_detail):
        ...
        trimap = generate_trimap(mask_to_run, w_detail_to_run)
    ...
    def guided_filter_matting(self, img, mask, fg_thresh, bg_thresh, erode_size, preserve_transparency=False, sharpness=0, focus_thresh=0.0, w_detail=None, disable_quality_loop=False, radius_field=None, material_maps=None):
        ...
        for _ in range(max_iter):
            ...
            mask_propagated = np.where((details_mask > 0) & (dist_outside < max_dist), dilated, mask_propagated)
        ...
        if is_pres_trans or disable_quality_loop or radius_field is None:
            ...
            q_small = color_guided_filter(I, p, r_small, eps=1e-3)
            q_large = color_guided_filter(I, p, r_large, eps=1e-3)
            ...
            q_hard = color_guided_filter(I, p, r_hard, eps=1e-3)
            q_soft = color_guided_filter(I, p, r_soft, eps=1e-3)
            q_detail = color_guided_filter(I, p, r_detail, eps=1e-3)
        ...
        if isinstance(preserve_transparency, np.ndarray):
            ...
            final_mask = (q * 255.0).astype(np.uint8)
...
def generate_trimap(mask, w_detail):
    """
...
def decontaminate_colors(img, mask, fg_thresh=None, hair_conf=None):
    ...
    # 1. Locate the active transition zone spatially (within 15px inside or 10px outside the mask edge)
    ...
    # Boundary zone is within 15px inside or 10px outside the mask edge
    transition_mask = (((dist_inside > 0) & (dist_inside <= 15)) | 
                       ((dist_outside > 0) & (dist_outside <= 10))).astype(np.uint8)
    ...
    # Kernels up to 31 are sufficient for local transition zone propagation at native resolution
    kernels = [3, 7, 15, 31]
    ...
    # 6. Blend recovered and prior foreground: favor prior at low alpha
    blend_alpha = np.clip((alpha_3d - 0.05) / 0.20, 0.0, 1.0)
    F_estimate = blend_alpha * F_recovered + (1.0 - blend_alpha) * F_prior
    
    # 7. Apply decontamination: modulate strength by hair confidence map or bright background status
    # On bright backgrounds, we force a high baseline (0.95) to strip out all white outlines.
    ...
    w_decon = np.maximum(hair_conf_3d * 0.95 + (1.0 - hair_conf_3d) * 0.10, is_bright_bg_3d * 0.95)
